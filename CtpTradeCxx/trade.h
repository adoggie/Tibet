...
//API
#include "ctpapi/x64_linux/ThostFtdcTraderApi.h"
...
//	Redis * redis_;
	std::string user_id_,password_,broker_id_,auth_code_,product_info_;
	int front_id_;
	int session_id_;
...
//	std::atomic< CThostFtdcTradingAccountField >  ftd_trade_account_;
    ...
    std::map< std::string , CThostFtdcOrderField > orders_ , orders_tmp_; // 当前委托记录
//    std::map< std::string , CThostFtdcTradeField > trade_;  // 成交列表
    std::vector< CThostFtdcTradeField > trades_;  // 成交列表
...
public:
    ...
    int cancelOrder(const OrderReturn& req);
