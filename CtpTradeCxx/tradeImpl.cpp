...
//    positions_[ task_data.InstrumentID] = task_data;
    positions_.push_back(task_data)  ;
//    if(task.task_last){
...
//    getStr(req, "BrokerID", myreq.BrokerID);
    ...
    if(ret){
        std::stringstream ss;
        ss<<"ReqQryTradingAccount Failed, Code: " << ret ;
        Application::instance()->getLogger().error(ss.str());
    }
...
int TdApi::sendOrder(const OrderRequest& req){
//    dict req;
//    req["InstrumentID"] = req.symbol;
/*
     req = {}
 ...
 */
    ...
    r.VolumeCondition = THOST_FTDC_VC_AV;  // 任何数量
    //# 判断FAK和FOK
    // FOK
    ...
    api->ReqOrderInsert(&r, r.RequestID);
...
// 撤单
int TdApi::cancelOrder(const OrderReturn& req){
...
//    self.reqOrderAction(req, cancelOrderReq.request_id)
    ...
    api->ReqOrderAction(&r,nextRequestId());
