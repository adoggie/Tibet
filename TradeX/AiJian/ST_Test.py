...
@revision:
 2019/5/24
  1. 买卖zf 价差比例参数放入mongodb 在  init_codes.py 初始。
"""
import json
import numpy as np
import talib as ta
    ...
    def do_zf(self, tick):
        ...
        if cs.oc_last != 'open':
            # for limit,pp in self.open_prices.items():
                ...
                if zf <= cs.buy_diff_rate :
                    num = cs.buy_qty
                    if stock.price.last < 8 : # 当前价小于8块
                        num *= 2

                    pos = self.getPosition(cs.code)
                    if pos.qty_td + num > cs.limit_buy_qty:
                        print code,' open num reached max-limitation!' , cs.limit_buy_qty
                        return
