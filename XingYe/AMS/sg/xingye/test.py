...
def init():
    ...
    orders = stbase.TradeManager().xy_proxy.get_orders().filter(code='0600008',serial=39917)
...
def on_tick(stock,stk):
    ...
    print_line('BuyPrice1:{} BuyQty1:{} SellPrice1:{} SellQty1:{}'.format(stk.BuyPrice1,
                                                                          stk.BuyQty1,
                                                                          stk.SellPrice1,
                                                                          stk.SellQty1))
    if not context.data.get('buy',False):
        ...
        print 'do buy()..',qty
        order_no = stbase.TradeManager().xy_proxy.buy(stock.code,stk.SellPrice1,qty)
