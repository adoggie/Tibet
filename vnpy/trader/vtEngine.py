...
class MainEngine(object):
    ...
    #----------------------------------------------------------------------
    def __init__(self, eventEngine,dataEngine = None):
        """Constructor"""
        # 记录今日日期
    ...
    #----------------------------------------------------------------------
    def connect(self, gatewayName,cfgs):
        """连接特定名称的接口"""
        gateway = self.getGateway(gatewayName)
        
        if gateway:
            gateway.connect(cfgs)
...
class DataEngine(object):
    ...
    def __init__(self, eventEngine):
        ...
        # 注册事件监听
        if app().prop('setting.DataEngine.event.enable',True):
            self.registerEvent()
    
    #----------------------------------------------------------------------
    def registerEvent(self):
        """注册事件监听"""
        if app().prop('setting.DataEngine.event.EVENT_CONTRACT.enable', True):
            self.eventEngine.register(EVENT_CONTRACT, self.processContractEvent)
        if app().prop('setting.DataEngine.event.EVENT_ORDER.enable',True):
            self.eventEngine.register(EVENT_ORDER, self.processOrderEvent)
        if app().prop('setting.DataEngine.event.EVENT_TRADE.enable',True):
            self.eventEngine.register(EVENT_TRADE, self.processTradeEvent)
        if app().prop('setting.DataEngine.event.EVENT_POSITION.enable', True):
            self.eventEngine.register(EVENT_POSITION, self.processPositionEvent)
        if app().prop('setting.DataEngine.event.EVENT_ACCOUNT.enable', True):
            self.eventEngine.register(EVENT_ACCOUNT, self.processAccountEvent)
        if app().prop('setting.DataEngine.event.EVENT_LOG.enable', True):
            self.eventEngine.register(EVENT_LOG, self.processLogEvent)
        if app().prop('setting.DataEngine.event.EVENT_ERROR.enable', True):
    ...
    def processContractEvent(self, event):
        ...
        from vnpy.zebra.trade import app
        app().invokeAction('onContract',contract)
