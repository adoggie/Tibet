...
class DrEngine(object):
    ...
    #----------------------------------------------------------------------
    def loadSetting2(self):
        from vnpy.zebra.trade import app

        f = open(self.settingFilePath)
        drSetting = json.load(f)

        subreqs = app().invokeAction('contract_subscribe')
        for req in subreqs:
            self.mainEngine.subscribe(req, req.gatewayName)
            # 订阅的合约当接收到tick时，gateway将其投入eventEngine,然后被注册的函数
            # processTickEvent()接收，最后被分派写入到DB
            vtSymbol = req.symbol
            ...
            if True:
                ...
                self.bgDict[vtSymbol] = BarGenerator(self.onBar) # 订阅生成１分钟的bar
            # else:
    ...
    def loadSetting(self):
        """加载配置
        在 bar配置项中配置的合约将生成1分钟的Bar，并被写入DB
        """
        with open(self.settingFilePath) as f:
            ...
            if 'tick' in drSetting:
                ...
                for setting in l:
                    ...
                    # 针对IB接口，订阅行情需要货币和产品类型 （数字货币)
                    if len(setting)>=5:
                        req.currency = setting[3]
                        req.productClass = setting[4]

                    self.mainEngine.subscribe(req, gateway) # 订阅发送
