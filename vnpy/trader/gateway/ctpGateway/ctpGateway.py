...
from base import *
from market import CtpMdApi
from trade import CtpTdApi
...
class CtpGateway(VtGateway):
    ...
    def connect(self,cfgs):
        ...
        brokerID = setting.get('brokerID')
        tdAddress = setting.get('tdAddress')
        mdAddress = setting.get('mdAddress')
        ...
        # 创建行情和交易接口对象
        self.mdApi.connect(userID, password, brokerID, mdAddress)
        # 只有指定了交易地址才连接交易服务器，以便于行情和交易分开部署
        if tdAddress:
            self.tdApi.connect(userID, password, brokerID, tdAddress, authCode, userProductInfo)
            # 交易api初始化并启动查询
            self.initQuery()
    ...
    def queryDepthMarketData(self,queryReq):
        self.tdApi.queryDepthMarketData(queryReq)

    def queryCommissionRate(self, queryReq):
        self.tdApi.queryCommissionRate(queryReq)
    #----------------------------------------------------------------------
...
class CtpTdApi(TdApi):
    ...
    def onRspQryInstrumentCommissionRate(self, data, error, n, last):
        ...
        commission.ExchangeID  = data['ExchangeID']
        self.gateway.onContractCommission(commission)
        pass
